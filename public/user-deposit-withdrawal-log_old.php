<?php
/**
 * Template Name: User Deposit Withdrawal Log
 * Description: Displays the deposit withdrawal log for the current user.
 */
...
if (!is_user_logged_in()) {
    wp_redirect(wp_login_url(home_url('/my-deposit-withdrawals/'))); // Redirect to login, then back here
    exit;
}
...
<div class="container smc-log-container">
    <h2><i class="fas fa-undo-alt"></i> سجل عمليات سحب الوديعة الخاص بك</h2>
    ...
    <table id="user-deposit-withdrawals-table" class="display compact stripe hover smc-log-table" style="width:100%">
        <thead>
            <tr>
                <th>تاريخ الطلب</th>
                <th>معرف العملية</th>
            ...
            if ($withdrawals) {
                foreach ($withdrawals as $withdrawal) {
                    ...
                    if ($withdrawal->status == 'pending') {
                        ...
                        echo '<button class="smc-button smc-button-danger cancel-withdrawal-button" data-id="' . esc_attr($withdrawal->id) . '" data-type="deposit_withdrawal"><i class="fas fa-times"></i> إلغاء</button>';
                    } else {
                ...
                echo '<tr><td colspan="7">لا توجد بيانات سحب ودائع حالية لعرضها.</td></tr>'; // Updated colspan
            }
            ?>
        </tbody>
    </table>
...
jQuery(document).ready(function($) {
    ...
    if ($.fn.DataTable) {
        try {
            $('#user-deposit-withdrawals-table').DataTable({
                responsive: true,
            ...
            console.error("Error initializing DataTables for user deposit withdrawals:", e);
            $('.smc-log-container').prepend('<p class="smc-error-message">حدث خطأ أثناء تحميل جدول السجلات التفاعلي.</p>');
        }
    } else {
        console.warn("DataTables library not found for user deposit withdrawals.");
    ...
    $('body').on('click', '.cancel-withdrawal-button', function() {
        ...
        var withdrawalType = button.data('type'); // Should be 'deposit_withdrawal'
