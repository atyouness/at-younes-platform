<?php
/**
 * Template Name: Users Profit Withdrawal Log (Admin)
 * Description: Displays the profit withdrawal Log for administrators.
 */
...
<div class="container smc-log-container">
    <h2><i class="fas fa-hand-holding-usd"></i> سجل سحب الأرباح (للمسؤول)</h2>
    ...
    <table id="admin-profit-withdrawals-table" class="display compact stripe hover smc-log-table" style="width:100%">
        <thead>
            <tr>
                <th>تاريخ الطلب</th>
                <th>معرف العملية</th>
            ...
            if ($withdrawals) {
                foreach ($withdrawals as $withdrawal) {
                    ...
                    if (!empty($withdrawal->approved_by)) {
                        $admin_info_pw = get_userdata($withdrawal->approved_by);
                        $admin_username = $admin_info_pw ? $admin_info_pw->user_login : __('مسؤول غير معروف', 'smc');
                    }
                    ...
                    if ($withdrawal->status == 'pending') {
                        ...
                        echo '<button class="smc-button approve-button" data-id="' . esc_attr($withdrawal->id) . '" data-type="profit_withdrawal"><i class="fas fa-check"></i> ' . __('موافقة', 'smc') . '</button>';
                        echo '<button class="smc-button reject-button" data-id="' . esc_attr($withdrawal->id) . '" data-type="profit_withdrawal"><i class="fas fa-times"></i> ' . __('رفض', 'smc') . '</button>';
                    } else {
                ...
                echo '<tr><td colspan="11">' . __('لا توجد طلبات سحب أرباح (غير ملغاة) لعرضها.', 'smc') . '</td></tr>'; // Updated colspan
            }
            ?>
        </tbody>
    </table>
...
jQuery(document).ready(function($) {
    ...
    if ($.fn.DataTable) {
        try {
            $('#admin-profit-withdrawals-table').DataTable({
                responsive: true,
                dom: 'Bfrtip',
            ...
            console.error("Error initializing DataTables for admin profit withdrawals log:", e);
            $('.smc-log-container').prepend('<p class="smc-error-message">حدث خطأ أثناء تحميل جدول السجلات التفاعلي.</p>');
        }
    } else {
        console.warn("DataTables library not found for admin profit withdrawals log.");
    ...
    // This uses the SAME JS code as in users-deposit-withdrawal-log.php
    // Ensure the smc_data object includes withdraw_profit_approval_nonce
    $('body').on('click', '.smc-button.approve-button, .smc-button.reject-button', function() {
        ...
        if (withdrawalType === 'deposit_withdrawal') {
            ...
            ajaxNonce = smc_data.withdraw_deposit_approval_nonce;
        } else if (withdrawalType === 'profit_withdrawal') {
            ajaxAction = 'smc_handle_profit_withdrawal_action';
            ajaxNonce = smc_data.withdraw_profit_approval_nonce; // Use the correct nonce
        } else {
...
<?php // Use the same styles as users-deposit-withdrawal-log.php ?>
<style>
